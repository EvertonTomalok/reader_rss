...
def crawl_noticias(iteravel, categoria):
    ...
    with tqdm(
        initial=0,
        total=tm,
        bar_format="{desc}: {percentage:3.0f}%|{bar}|{n_fmt}/{total_fmt} > Time: {elapsed}",
    ) as pbar:
    ...
    if total > 0:
        ...
        while True:
            ...
            # Titulo da noticia
            frase_a_direita = (
                "\033[;1mNotícia atual:\033[m " + str(contador + 1) + "/" + str(total)
            )
            print(f"\n\n\n \033[;1m Título: {news[contador][0]}  \033[m ", end="\n")
            print(
                f"->\033[;1m Data de postagem: \033[m{data_postagem} {frase_a_direita:>70}",
                end="\n\n\n",
            )
